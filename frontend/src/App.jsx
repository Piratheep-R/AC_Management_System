...
function App() {
  ...
  const acState = isApiEnabled() ? systemInfo.acState : meta.acState;
  const controlledTemperature = isApiEnabled() ? systemInfo.temperature : meta.temperature;
  const acOn = acState === 'ON';
  const acStarting = systemInfo.controlPending
    && acState === 'OFF'
      ...
      <main className="dashboard" id="top">
        ...
        <section className="metric-grid" aria-label="Current smart classroom metrics">
          <MetricCard
            ...
            value={meta.label}
            description={isApiEnabled()
              ? `${personCount} ${personCount === 1 ? 'person' : 'people'} detected · ${Math.round(confidence * 100)}% confidence`
          ...
          <MetricCard
            ...
            description={systemInfo.controlPending
          ...
          <MetricCard
            ...
            description="Total runtime in this demo session"
            icon="clock"
            tone="purple"
        ...
        <section className="analytics-grid" id="analytics">
          ...
          <Panel className="rules-panel" id="automation-rules" title="Automation Rules" icon="shield">
            <AutomationRules current={occupancy} onSelect={manualClassChange} />
          </Panel>
        </section>
...
function Header({ onToggleSidebar, apiMode, apiError, onAction }) {
  ...
  return (
    <div className="chart-wrap">
      ...
      <div className="chart-legend">
        ...
        <span><i className="legend-dot low" />LOW (1–2)</span>
        <span><i className="legend-dot medium" />MEDIUM (3–9)</span>
        <span><i className="legend-dot high" />HIGH (10+)</span>
      </div>
    </div>
  );
}
...
function AutomationRules({ current, onSelect }) {
  const rules = [
    ['EMPTY', '0 people', 'AC OFF', 'slate'],
    ['LOW', '1–2 people', 'AC OFF', 'green'],
    ['MEDIUM', '3–9 people', 'AC ON · 24°C', 'blue'],
    ['HIGH', '10+ people', 'AC ON · 20°C', 'red'],
    ['JANITOR', 'Cleaning activity', 'AC OFF', 'purple'],
  ];
  ...
  return (
    <div className="rule-list">
      {rules.map(([name, range, action, tone]) => (
        <button
          type="button"
          className={`rule-item tone-${tone} ${current === name ? 'selected' : ''}`}
          key={name}
          onClick={() => onSelect(name)}
        >
          <span className="rule-icon"><Icon name={name === 'JANITOR' ? 'broom' : 'users'} size={20} /></span>
      ...
      <div className="rules-active"><span className="status-dot success" /> Rules are active</div>
    </div>
  );
}
